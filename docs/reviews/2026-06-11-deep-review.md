...
- **🟠 [SECURITY — JWT в localStorage] — отложено (нужна координация с бэкендом).** Бэкенд уже поддерживает `AUTH_REFRESH_COOKIE` (httpOnly-cookie для refresh: `TokenResponse.refresh_token=None`, `RefreshRequest` берёт токен из cookie — `app/schemas/user.py`). Полный переход требует включения флага на бэкенде + `withCredentials` + e2e-проверки всего flow; вслепую не меняю (риск сломать авторизацию). Оставлен как трекнутый пункт с готовым путём включения.
