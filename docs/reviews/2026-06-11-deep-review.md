...
- **🟠 [SECURITY — JWT в localStorage] — ИСПРАВЛЕНО (миграция на httpOnly-куки, 2026-06-12).** Бэкенд перешёл на доставку токенов в httpOnly-куках по умолчанию (Secure вне debug, `SameSite=Strict`, refresh ограничен `path=/auth`, тело ответа — `null`; body-режим оставлен для будущего React Native через заголовок `X-Token-Delivery: body`). Добавлен CSRF-middleware (чужой Origin на мутациях → 403). Фронт переведён на cookie-режим:
  ...
  - **⚠️ ТРЕБУЕТСЯ на бэкенде:** в dev `.env` выставить **`COOKIE_PATH_PREFIX=/api`** (сейчас дефолт `""`). Иначе refresh-кука получает `path=/auth`, а фронт через прокси ходит на `/api/auth/refresh` — кука не отправляется, и после истечения access-токена пользователя выбрасывает на логин. Access-кука (`path=/`) работает в обоих случаях, поэтому симптом отложенный. Также проверить, что CSRF-middleware в dev пропускает Origin `http://localhost:8082`.
